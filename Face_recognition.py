...
def predict_image_realtime(image1, model, path_name='./loss'):
    ...
    for dir_item in os.listdir(path_name):
        ...
        if dir_item.endswith('.txt'):
            ...
            print(dir_item + ':', probability)  # print 当前帧和每一个图像库里的特征向量的loss
...
if __name__ == '__main__':
    ...
    while True:
        ret, frame = cap.read()
        # cap.read() captures a frame from the video source, which is stored in the frame variable
        font = cv2.FONT_HERSHEY_SIMPLEX

        bboxes, _ = handler.detector.detect(frame)

        # 用了retinaface_mnet025的detect模块

        print(bboxes, bboxes.shape[0],type(bboxes),'what is bboxes')
        if bboxes.shape[0] == 0:
